...
export type RoomMeta = { code: string; gameType?: 'uno' | 'ludo' | 'snake'; isPrivate: boolean; players: number; bots: number; maxPlayers: number; status: string; hostId: string; host?: string; rules: { stacking: boolean; sevenZero: boolean; jumpIn: boolean }; createdAt: number };
export type LudoToken = { id: string; color: Color; pos: number };
...
export type GameSnapshot = {
  roomId: string; gameType?: 'uno' | 'ludo' | 'snake'; status: 'waiting' | 'playing' | 'round-over' | 'match-over'; players: { id: string; username: string; avatarUrl?: string; avatarPreset?: string; ready: boolean; connected: boolean; isBot: boolean; isHost?: boolean; joinedAt?: number; score: number; handCount: number; unoCalled: boolean; isYou: boolean; ludoColor?: 'red' | 'green' | 'yellow' | 'blue'; disconnectDeadline?: number }[];
